...
def precompute_rotary_emb(dim, max_positions):
    ...
    for t in range(max_positions):
        for i in range(dim // 2):
            ...
            rope_cache[t, i, 0] = cos_val
            rope_cache[t, i, 1] = sin_val          
    ### END YOUR CODE ###
    return rope_cache
...
def apply_rotary_emb(x, rope_cache):
    ...
    x = x.view(x.shape[0], x.shape[1], x.shape[2], x.shape[3] // 2, 2)
    rotated_x = torch.view_as_complex(rope_cache) * torch.view_as_complex(x)
    rotated_x = torch.view_as_real(rotated_x)
    rotated_x = rotated_x.reshape(x.shape[0], x.shape[1], x.shape[2], x.shape[3] * 2)
    ### END YOUR CODE ###
    return rotated_x
