...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if(transition == "LA"):
            fir = self.stack.pop()
            sec = self.stack.pop()
            self.dependencies.append((fir, sec))
            self.stack.append(fir)
        if(transition == "RA"):
            fir = self.stack.pop()
            sec = self.stack.pop()
            self.dependencies.append((sec, fir))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    """
    dependencies = []
    partical_parses = [PartialParse(sentence) for sentence in sentences]
    unfinished_parses = partical_parses[:]
    while len(unfinished_parses) > 0:
        parses = unfinished_parses[:batch_size]
        transitions = model.predict(parses)
        for i, transition in enumerate(transitions):
            parses[i].parse_step(transition)
            if len(parses[i].buffer) == 0 and len(parses[i].stack) == 1:
                #dependencies.append(parses[i].dependencies)
                unfinished_parses.remove(parses[i])
    dependencies = [pp.dependencies for pp in partical_parses]
