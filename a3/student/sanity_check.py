...
def generate_outputs(model, source, target, vocab):
    ...
    source_padded = model.vocab.src.to_input_tensor(source, device)
    target_padded = model.vocab.tgt.to_input_tensor(target, device)
...
def question_1d_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    source_padded = model.vocab.src.to_input_tensor(src_sents, device)
